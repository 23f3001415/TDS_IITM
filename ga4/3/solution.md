# Question 3: dbt - Customer Analytics Model
...
1. Pull carts and orders from upstream staging models using `{{ ref() }}`.
2. Keep only carts from the last 30 days.
3. Convert cart timestamps to weekly buckets with `date_trunc('week', created_at)`.
4. Left join carts to orders using `cart_id`.
5. If `order_id` is missing, mark the cart as abandoned.
6. Return the prepared rows for downstream mart models.
